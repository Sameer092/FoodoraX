...
function ManageRestaurants({ navigation, getAllRestaurants, setRestaurantVerified }) {
  const [rows, setRows] = useState([]);
  const [tab, setTab] = useState('pending');

  const load = useCallback(() => { getAllRestaurants().then(setRows); }, []);
  ...
  const toggle = (r) => {
    ...
    Alert.alert(verify ? 'Approve Restaurant' : 'Revoke Approval', `${verify ? 'Approve' : 'Revoke'} "${r.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Confirm', onPress: async () => { await setRestaurantVerified(r.id, verify); load(); } },
    ]);
  };

  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <Header title="Restaurants" onBack={() => navigation.goBack()} />
      ...
      <FlatList
        ...
        ListEmptyComponent={<EmptyState icon="storefront-outline" title="No restaurants" description="Nothing here yet" />}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <View style={styles.cardTop}>
              <View style={{ flex: 1 }}>
                <Text style={styles.name}>{item.name}</Text>
                <Text style={styles.owner}>{item.owner && item.owner.full_name}</Text>
            ...
            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: item.is_verified ? colors.dangerSoft : colors.primary }]} onPress={() => toggle(item)}>
              ...
              <Text style={[styles.actionText, { color: item.is_verified ? colors.danger : colors.white }]}>{item.is_verified ? 'Revoke Approval' : 'Approve'}</Text>
            </TouchableOpacity>
          </View>
        )}
      />
...
export default enhancer(ManageRestaurants);
